...
export function HomeworkView({ student, accessToken = '', childId = '', studentSession = false }: { student: StudentProfile; accessToken?: string; childId?: string; studentSession?: boolean }) {
  ...
  async function analyze() {
    ...
    try {
      ...
      const data = await apiPostForm<{ feedback: string; provider: string }>('/api/homework/lightweight-feedback', payload, { Authorization: `Bearer ${accessToken}`, ...(studentSession ? {} : { 'x-access-mode': 'child' }) });
  ...
  return <div className="page-stack narrow">
    ...
    <div className="form-card">
      ...
      <button className="primary-button" onClick={analyze} disabled={loading}>{loading ? 'Uploading...' : 'Take a Photo or Upload'}</button>
