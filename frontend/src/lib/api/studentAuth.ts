...
export function studentLogin(username: string, pin: string): Promise<StudentSession> {
  return apiPost<StudentSession>('/student/login', { username: username.trim().toLowerCase(), pin });
}
